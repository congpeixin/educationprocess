...
object streamingProcessToDB {
  ...
  def replaceContext(str: String): JSONObject ={
    ...
    if (json.has("content_text")){
      ...
      val content_html = json.get("content_html").toString.replaceAll("<span.*><b>未按照规范转载者，虎嗅保留追究相应责任的权利</b></span>|<span>html<br></span>|<span.*>\\*文章为作者独立观点，不代表虎嗅网立场<br></span>" +
